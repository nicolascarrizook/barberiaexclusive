-- =====================================================
-- MIGRATION: Barber Invitations System
...
CREATE INDEX IF NOT EXISTS idx_barber_invitations_barbershop_id ON barber_invitations(barbershop_id);
CREATE INDEX IF NOT EXISTS idx_barber_invitations_status ON barber_invitations(status);
CREATE INDEX IF NOT EXISTS idx_barber_invitations_invitation_code ON barber_invitations(invitation_code) WHERE invitation_code IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_barber_invitations_email ON barber_invitations(email) WHERE email IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_provisional_barbers_barbershop_id ON provisional_barbers(barbershop_id);
CREATE INDEX IF NOT EXISTS idx_provisional_barbers_invitation_id ON provisional_barbers(invitation_id);
...
DROP POLICY IF EXISTS "Owners can view barbershop invitations" ON barber_invitations;
DROP POLICY IF EXISTS "Owners can create barbershop invitations" ON barber_invitations;
DROP POLICY IF EXISTS "Owners can update barbershop invitations" ON barber_invitations;
DROP POLICY IF EXISTS "Users can view invitations by code" ON barber_invitations;
DROP POLICY IF EXISTS "Users can claim invitations" ON barber_invitations;
DROP POLICY IF EXISTS "Owners can view provisional barbers" ON provisional_barbers;
DROP POLICY IF EXISTS "Owners can create provisional barbers" ON provisional_barbers;
DROP POLICY IF EXISTS "Owners can update provisional barbers" ON provisional_barbers;
DROP POLICY IF EXISTS "Owners can delete provisional barbers" ON provisional_barbers;
...
DROP TRIGGER IF EXISTS set_invitation_code_trigger ON barber_invitations;
...
DROP TRIGGER IF EXISTS update_barber_invitations_updated_at ON barber_invitations;
CREATE TRIGGER update_barber_invitations_updated_at
    BEFORE UPDATE ON barber_invitations
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_provisional_barbers_updated_at ON provisional_barbers;
...
-- This should return the table names if everything was created successfully
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('barber_invitations', 'provisional_barbers');